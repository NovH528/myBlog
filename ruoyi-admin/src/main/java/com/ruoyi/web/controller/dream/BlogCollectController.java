package com.ruoyi.web.controller.dream;

import java.util.List;
import javax.servlet.http.HttpServletResponse;
import org.springframework.security.access.prepost.PreAuthorize;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;
import com.ruoyi.common.annotation.Log;
import com.ruoyi.common.core.controller.BaseController;
import com.ruoyi.common.core.domain.AjaxResult;
import com.ruoyi.common.enums.BusinessType;
import com.ruoyi.dream.domain.BlogCollect;
import com.ruoyi.dream.service.IBlogCollectService;
import com.ruoyi.common.utils.poi.ExcelUtil;
import com.ruoyi.common.core.page.TableDataInfo;

/**
 * 博客收藏Controller
 * 
 * @author ruoyi
 * @date 2025-08-17
 */
@RestController
@RequestMapping("/dream/collect")
public class BlogCollectController extends BaseController
{
    @Autowired
    private IBlogCollectService blogCollectService;

    /**
     * 查询博客收藏列表
     */
    @PreAuthorize("@ss.hasPermi('dream:collect:list')")
    @GetMapping("/list")
    public TableDataInfo list(BlogCollect blogCollect)
    {
        startPage();
        List<BlogCollect> list = blogCollectService.selectBlogCollectList(blogCollect);
        return getDataTable(list);
    }

    /**
     * 导出博客收藏列表
     */
    @PreAuthorize("@ss.hasPermi('dream:collect:export')")
    @Log(title = "博客收藏", businessType = BusinessType.EXPORT)
    @PostMapping("/export")
    public void export(HttpServletResponse response, BlogCollect blogCollect)
    {
        List<BlogCollect> list = blogCollectService.selectBlogCollectList(blogCollect);
        ExcelUtil<BlogCollect> util = new ExcelUtil<BlogCollect>(BlogCollect.class);
        util.exportExcel(response, list, "博客收藏数据");
    }

    /**
     * 获取博客收藏详细信息
     */
    @PreAuthorize("@ss.hasPermi('dream:collect:query')")
    @GetMapping(value = "/{id}")
    public AjaxResult getInfo(@PathVariable("id") Long id)
    {
        return success(blogCollectService.selectBlogCollectById(id));
    }

    /**
     * 新增博客收藏
     */
    @PreAuthorize("@ss.hasPermi('dream:collect:add')")
    @Log(title = "博客收藏", businessType = BusinessType.INSERT)
    @PostMapping
    public AjaxResult add(@RequestBody BlogCollect blogCollect)
    {
        return toAjax(blogCollectService.insertBlogCollect(blogCollect));
    }

    /**
     * 修改博客收藏
     */
    @PreAuthorize("@ss.hasPermi('dream:collect:edit')")
    @Log(title = "博客收藏", businessType = BusinessType.UPDATE)
    @PutMapping
    public AjaxResult edit(@RequestBody BlogCollect blogCollect)
    {
        return toAjax(blogCollectService.updateBlogCollect(blogCollect));
    }

    /**
     * 删除博客收藏
     */
    @PreAuthorize("@ss.hasPermi('dream:collect:remove')")
    @Log(title = "博客收藏", businessType = BusinessType.DELETE)
	@DeleteMapping("/{ids}")
    public AjaxResult remove(@PathVariable Long[] ids)
    {
        return toAjax(blogCollectService.deleteBlogCollectByIds(ids));
    }
}
